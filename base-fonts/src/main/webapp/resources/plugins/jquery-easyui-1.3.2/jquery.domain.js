 ...
 * numberbox: numberboxname and visible
 */
(function($) {
    /**
     * 收集参数
    ...
    function build(target) {
    	...
    	var t = $(target);
    	var fn;
    	if(t.attr("queryHandler") || t.attr("queryFunction")) {
    		fn = eval(t.attr("queryHandler") || t.attr("queryHandler"));
    	}
    	t.find(":input[name]:visible").each(function(ndx, dom) {
    		var box = $(dom);
    		//将要转换为combobox/combotree的dom对象
			if (box.hasClass("easyui-combobox-d") || box.hasClass("easyui-combotree-d") || box.hasClass("easyui-combobox-disable") || box.hasClass("easyui-combotree-disable")) {
				var c = box.hasClass("easyui-combobox-d") || box.hasClass("easyui-combobox-disable") ? "combobox" : "combotree";
				box[c]({
					keyHandler: {
						enter: function() {
							var panel = box.combo("panel");
							if (c == "combobox") {
								var item;
								if (panel.is(":visible")) {
									...
									if (item.length > 0) {
										...
										box.combobox("hidePanel");
									}
								} else {
									box.combobox("setValues", box.combobox("getValues"));
									cascadeHandler(dom);
									enterHandler(dom, fn);
								}
							} else if (c == "combotree") {
					...
					onSelect: function(record) {//适用于单选或多选combobox，以及单选或多选combotree
						if (c == "combobox") {
							cascadeHandler(dom);
							autoHandler(dom, fn);
						} else if (c == "combotree") {
							var opts = box.combotree("options");
							if (opts.multiple == false) {//适用于单选combotree
								cascadeHandler(dom);
								autoHandler(dom, fn);
							}
						}
					},
					onUnselect: function(record) {//适用于多选combobox
						cascadeHandler(dom);
						autoHandler(dom, fn);
					},
					onCheck: function(node, checked) {//适用于多选combotree
						cascadeHandler(dom);
						autoHandler(dom, fn);
					},
			...
			else if(box.hasClass("easyui-datebox-d") || box.hasClass("easyui-datetimebox-d") || box.hasClass("easyui-datebox-disable") || box.hasClass("easyui-datetimebox-disable")) {
				box.width(box.outerWidth());
				var c = box.hasClass("easyui-datebox-d") || box.hasClass("easyui-datebox-disable") ? "datebox" : "datetimebox";
				box[c]({
					keyHandler: {
						enter: function() {
							var panel = box.combo("panel");
			...
			else if(box.hasClass("easyui-numberspinner-d") || box.hasClass("easyui-timespinner-d") || box.hasClass("easyui-numberspinner-disable") || box.hasClass("easyui-timespinner-disable")) {
				box.removeClass("input").width(box.outerWidth());
				var c = box.hasClass("easyui-numberspinner-d") || box.hasClass("easyui-numberspinner-disable") ? "numberspinner" : "timespinner";
				box[c]({
					onSpinUp: function() {
						autoHandler(dom, fn);
					},
			...
			else if(box.hasClass("easyui-numberbox-d") || box.hasClass("easyui-validatebox-d") || box.hasClass("easyui-numberbox-disable") || box.hasClass("easyui-validatebox-disable")) {
				var c = box.hasClass("easyui-numberbox-d") || box.hasClass("easyui-numberbox-disable") ? "numberbox" : "validatebox";
				box[c]();
				box.unbind("keyup").bind("keyup", {}, function(event) {
                    if (event.keyCode == 13) {
                    	//box.blur();
    	...
    	function enterHandler(target, fn) {
    		if(!fn) {
				return;
			}
			var t = $(target);
			var f = t.attr("enterQuery");
			if (f && f.toLowerCase() == "true") {
				window.setTimeout(function() { fn(); }, 0);
			}
    	}
    	function autoHandler(target, fn) {
    		if(!fn) {
				return;
			}
			var t = $(target);
			var f = t.attr("autoQuery");
			if (f && f.toLowerCase() == "true") {
				window.setTimeout(function() { fn(); }, 0);
			}
    	}
        ...
        function cascadeHandler(target) {
        	...
        	if(box.hasClass("easyui-combobox-d") || box.hasClass("easyui-combotree-d") || box.hasClass("easyui-combobox-disable") || box.hasClass("easyui-combotree-disable")) {
        		var type = box.hasClass("easyui-combobox-d") || box.hasClass("easyui-combobox-disable") ? "combobox" : "combotree";
            	box[type]("loadData", []);
            	box[type]("setValue", "");
            	box.each(function(ndx, ele) {
            		$.data(this, type).loaded = false;
            	});
        	}
        }
    }
    ...
    function load(target, options) {
    	...
    	build(target);
    	var t = $(target);
    	if(options && options.url && new RegExp("/(-?[0-9]+)").test(options.url)) {
    ...
    function datagrid(target, options) {
        ...
        $(target).datagrid($.extend({}, opts, {
        	...
        	onLoadSuccess: function(data, status, XHR) {
                ...
                panel.find("div.datagrid-body").bind("click", function(e) {
                	e.stopPropagation();
                	if ($(e.target).closest("table.datagrid-btable").length == 0) {
                		t.datagrid("clearSelections");
                	};
                });
    	...
    	$.ajax({
            ...
            success: function(data, status, XHR) {
            	...
            	for(var j = 0; j < divs.length; j++) {
            		...
            		if(!mch) {
            			div.html("<font style='color:red'>不存在</font>");
            		}
            	}
                window.setTimeout(function() { opts.onLoadSuccess.call(target, data, status, XHR); }, 800);
            },
    ...
    $.fn.domain.defaults = {
        ...
        datagrid: $.extend({}, $.fn.datagrid.defaults, {
            ...
            toolbar: [{
                ...
                disabled: false,
                handler: function() {
                	var t = $(this).parents("div.datagrid-toolbar:first").next().find(">table:hidden");
                    var opts = t.datagrid("options");
                    var title = opts.title || opts.subject;
